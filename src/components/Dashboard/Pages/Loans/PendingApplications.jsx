...
const PendingApplications = () => {
  ...
  const {
    ...
    queryKey: ["pendingApplications", user?.email],
    queryFn: async () => {
      const { data } = await axiosSecure(`/pending-applications`);
      return data;
    },
  });

  useEffect(() => {
    document.title = "PENDING APPLICATIONS | ARTHI";
  }, []);
  ...
  return (
    <>
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {/* | Loan ID | User Info (Borrower name and email) | Amount | Date | Actions | */}
      ...
      <RejectApplicationModal
        isOpen={isRejectOpen}
        closeModal={() => setIsRejectOpen(false)}
        refetch={refetch}
        application={selectedApplication}
      />
